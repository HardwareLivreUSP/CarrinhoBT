...
void setup()  {
  pinMode(Tras1, OUTPUT);
  pinMode(Tras2, OUTPUT);  
...
void getJoystickState(byte data[8])    {
  ...
  if(joyX<-100 || joyX>100 || joyY<-100 || joyY>100)     return;      // commmunication error
     ...
     if(joyX>50){
        digitalWrite(Frente1, LOW);
        digitalWrite(Frente2, HIGH);
      }
      else if(joyX<-50){
        digitalWrite(Frente1, HIGH);
        digitalWrite(Frente2, 0);
      }
      else {
        digitalWrite(Frente1, 0);
        digitalWrite(Frente2, 0);
      }
      
      if( joyY >= 0 ){
        analogWrite(Tras1, joyY*2.55);
        analogWrite(Tras2, LOW);
      }
      else{
        analogWrite(Tras1, LOW);
        analogWrite(Tras2, -1*joyY*2.55);
      }
